...
<form method="post" name="applicationForm" action="" class="application-form">
	<input type="hidden" name="orderBy" value="{$orderBy|escape:'html'}" />
	<input type="hidden" name="order" value="{$order}" />
	...
	<div class="navigation-application top-nav">
	...
	<table border="0" cellpadding="0" cellspacing="0" class="tableSearchResultApplications" width="100%">
		...
		{foreach item=application from=$applications name=applications}
			<tr id="trApplication_{$application.id}" {if !$application.note}class="table-application-border-bottom"{/if}>
				...
				<td class="ApplicationPointedInListingInfo">
					...
					{if $application.comments}
						<a style="cursor: pointer;" onclick="showCoverLetter('{$application.id}')">[[View Cover Letter]]</a>
						<br />
			...
			{if $application.note}
				<tr>
					<td {if $acl->isAllowed('use_screening_questionnaires')}colspan="10"{else}colspan="9"{/if} class="ApplicationPointedInListingInfo" style="border-bottom: 1px solid #B2B2B2;">
...
<script type="text/javascript">
	...
	function modifyNote(noteId, url) {
		$.get(url, function(data) {
			...
			$("#trApplication_" + noteId).removeClass("table-application-border-bottom");
