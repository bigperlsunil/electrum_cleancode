			...
			<div class="searchResultsHeaderLineNew">
				...
				<div class="after-quick-links">
					{if $acl->isAllowed('apply_for_a_job')}
						...
						{else}
							{if isset($listing.ApplicationSettings.add_parameter) && $listing.ApplicationSettings.add_parameter == 2}
								{capture assign='applyBtn_onClick'}onclick="javascript:window.open('{if $listing.user.isJobg8}{$GLOBALS.site_url}/apply-now-external/?listing_id={$listing.id}{else}{$listing.ApplicationSettings.value}{/if}');"{/capture}
							{else}
								{capture assign='applyBtn_onClick'}onclick="popUpWindow('{$GLOBALS.site_url}/apply-now/?listing_id={$listing.id}&ajaxRelocate=1', 600, '<span>[[Apply Now]]</span>: {$listing.Title|addslashes|escape}')"{/capture}
							{/if}
						{/if}
					{elseif $acl->getPermissionParams('apply_for_a_job') == "message"}
						{capture assign='applyBtn_onClick'}onclick="popUpWindow('{$GLOBALS.site_url}/access-denied/?permission=apply_for_a_job', 400, '<span>[[Apply Now]]</span>: {$listing.Title|escape}')"{/capture}
					...
					{if isset($applyBtn_onClick)}
						<input type="button" class="buttonApply" {$applyBtn_onClick} value='[[Apply Now]]' />
					{/if}
					<ul>
						{if $GLOBALS.user_page_uri != "/my-job-details/" && $GLOBALS.user_page_uri != "/job-preview/"}
							{if $acl->isAllowed('save_job')}
								<li class="panelSavedIco"><a href="{$GLOBALS.site_url}/saved-ads/?listing_id={$listing.id}" onclick="popUpWindow('{$GLOBALS.site_url}/saved-ads/?listing_id={$listing.id}&displayForm=1', 400, '[[Save this Job]]', true, {if $GLOBALS.current_user.logged_in}true{else}false{/if}); return false;"  class="action">[[Save this Job]]</a></li>
							{elseif $acl->getPermissionParams('save_job') == "message"}
								<li class="panelSavedIco"><a href="{$GLOBALS.site_url}/saved-ads/?listing_id={$listing.id}"  onclick="popUpWindow('{$GLOBALS.site_url}/access-denied/?permission=save_job', 400, '[[Save this Job]]'); return false;" class="action">[[Save this Job]]</a></li>
							{/if}
							{if $GLOBALS.current_user.logged_in}
								{if $acl->isAllowed('save_job')}
									<li class="panelViewDitailsIco"><a href="{$GLOBALS.site_url}/saved-jobs/" class="action">[[View Saved Jobs]]</a></li>
								{elseif $acl->getPermissionParams('save_job') == "message"}
									<li class="panelViewDitailsIco"><a href="{$GLOBALS.site_url}/saved-jobs/" class="action" onclick="popUpWindow('{$GLOBALS.site_url}/access-denied/?permission=save_job', 400, '[[View Saved Jobs]]'); return false;">[[View Saved Jobs]]</a></li>
								{/if}
							{else}
									<li class="panelViewDitailsIco"><a onclick="popUpWindow('{$GLOBALS.site_url}/saved-listings/?listing_type_id=job', 400, 'Saved jobs'); return false;" href="{$GLOBALS.site_url}/saved-listings">[[View Saved Jobs]]</a></li>
							{/if}
						{/if}
						...
						{if $acl->isAllowed('flag_job')}
							<li class="flag-listing-ico"><a href="{$GLOBALS.site_url}/flag-listing/?listing_id={$listing.id}" onclick="popUpWindow('{$GLOBALS.site_url}/flag-listing/?listing_id={$listing.id}', 500, '[[Flag This Job]]'); return false;">[[Flag This Job]]</a></li>
						{elseif $acl->getPermissionParams('flag_job') == "message"}
							<li class="flag-listing-ico"><a href="{$GLOBALS.site_url}/flag-listing/?listing_id={$listing.id}" onclick="popUpWindow('{$GLOBALS.site_url}/access-denied/?permission=flag_job', 400, '[[Flag This Job]]'); return false;">[[Flag This Job]]</a></li>
						{/if}
						{if $GLOBALS.user_page_uri == "/my-job-details/"}
							<li class="print-listing-ico"><a target="_blank" href="{$GLOBALS.site_url}/print-my-job/?listing_id={$listing.id}">[[Print This Ad]]</a></li>
						{else}
						...
						{if $listing.latitude && $listing.longitude && (!$GLOBALS.settings.cookieLaw || $smarty.cookies.cookiePreferences != "System")}
							<li class="viewMapIco"><a href="{$GLOBALS.site_url}/display-job-map/?listing_id={$listing.id}&amp;searchId={$searchId}&amp;view=map" onclick="popUpWindowIframe('{$GLOBALS.site_url}/display-job-map/?listing_id={$listing.id}&amp;searchId={$searchId}&amp;view=map&amp;lightbox=1', 810, 715, '[[Map]]'); return false;">[[Map View]]</a></li>
						{/if}
					</ul>
					<div class="clr"></div>
				</div>
				...
				<div class="comp-profile-content">
					{if $listing.anonymous != 1 || $applications.anonymous === 0 }
						{if $listing.user.Logo.file_url}
							<div class="text-center"><img src="{$listing.user.Logo.file_url}" alt="" /></div>
						{/if}
						...
						{else}
							...
							{if $acl->isAllowed('use_private_messages')}
								<span class="link"><a href="{$GLOBALS.site_url}/private-messages/contacts/?pm_action=save_contact&amp;user_id={$listing.user.id}" onclick="popUpWindow('{$GLOBALS.site_url}/private-messages/contacts/?pm_action=save_contact&amp;user_id={$listing.user.id}', {if $GLOBALS.current_user.logged_in}400{else}500{/if}, '[[Save Contact]]', true, {if $GLOBALS.current_user.logged_in}true{else}false{/if}); return false;"  class="pmSendLink">[[Save Contact]]</a></span><br />
							{elseif $acl->getPermissionParams('use_private_messages') == "message"}
								<span class="link"><a href="#" onclick="popUpWindow('{$GLOBALS.site_url}/access-denied/?permission=use_private_messages', 400, '<span>[[Apply Now]]</span>: {$listing.Title|escape:"html"}'); return false;"  class="pmSendLink">[[Save Contact]]</a></span><br />
							{/if}
		...
		{if $acl->isAllowed('apply_for_a_job')}
			...
			{else}
				{if isset($listing.ApplicationSettings.add_parameter) && $listing.ApplicationSettings.add_parameter == 2}
					{capture assign='applyBtn_onClick'}onclick="javascript:window.open('{if $listing.user.isJobg8}{$GLOBALS.site_url}/apply-now-external/?listing_id={$listing.id}{else}{$listing.ApplicationSettings.value}{/if}');"{/capture}
				{else}
					{capture assign='applyBtn_onClick'}onclick="popUpWindow('{$GLOBALS.site_url}/apply-now/?listing_id={$listing.id}&ajaxRelocate=1', 600, '<span>[[Apply Now]]</span>: {$listing.Title|escape}')"{/capture}
				{/if}
			{/if}
		{elseif $acl->getPermissionParams('apply_for_a_job') == "message"}
			{capture assign='applyBtn_onClick'}onclick="popUpWindow('{$GLOBALS.site_url}/access-denied/?permission=apply_for_a_job', 400, '<span>[[Apply Now]]</span>: {$listing.Title|escape}')"{/capture}
