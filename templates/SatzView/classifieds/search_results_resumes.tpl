	...
	<div id="topResults">
		<div class="headerBgBlock">
			{if isset($search_criteria.username.value)}
				...
				{/if}
			{else}
				<div class="Results results-title">[[Resume Search Results]]</div><span></span>
			{/if}
			<!-- TOP QUICK LINKS -->
				...
				<ul>
					...
					<li class="savedIco">
						{if $GLOBALS.current_user.logged_in}
							...
							{/if}
						{else}
							<a onclick="popUpWindow('{$GLOBALS.site_url}/saved-listings/?listing_type_id=resume', 400, '{capture name="savedResumesPopup"}[[Saved resumes]]{/capture}{$smarty.capture.savedResumesPopup|escape:'quotes'}'); return false;" href="{$GLOBALS.site_url}/saved-listings/">[[Saved resumes]]</a>
						{/if}
					</li>
					<li class="savedIco">
						{if $GLOBALS.current_user.logged_in}
	...
	<div class="results-paging">
		...
		<div class="topNavBar">
			<div class="numberResults">
				...
				<p>[[Results:]] <span>{$listings_number}</span> {if $listings_number == 1}[[Resume]]{else}[[resumes]]{/if}</p>
		...
		{if $listings}
			...
			{else}
					...
					{foreach from=$listings item=listing name=listings}
						...
						{else}
							<tr {if $listing.priority == 1}class="priorityListing"{else}class="{cycle values = 'evenrow,oddrow' advance=true}"{/if}>
								<td>
									<div class="listing-section {if $show_brief_or_detailed == 'brief'}brief-view{/if}">
										...
										<div class="listing-info">
											<div class="left-side">
												<span><a>{if $listing.anonymous == 1}[[Anonymous User]]{else}<span class="longtext-25">{$listing.user.FirstName}</span> <span class="longtext-25">{$listing.user.LastName}</span>{/if}</a></span>
												{locationFormat location=$listing.Location format="short"}
												<br/><br/>[[Experience]]: <span class="strong"> {if $listing.TotalYearsExperience>0}{$listing.TotalYearsExperience}</span> [[years]]{/if}
											...
											<div class="right-side resume-side">
												<span>[[Posted]]: [[$listing.activation_date]]</span>
											</div>
										</div>
				...
				<div class="pageNavigation">
					<span class="prevBtn"><img src="{image}page-prev.png" alt="[[Previous]]"/>
						{if $listing_search.current_page-1 > 0}<a href="?searchId={$searchId}&amp;action=search&amp;page={$listing_search.current_page-1}&amp;view={$view}">[[Previous]]</a>{else}<a>[[Previous]]</a>{/if}
					</span>
					<span class="navigationItems">
						{if $listing_search.current_page-3 > 0}<a href="?searchId={$searchId}&amp;action=search&amp;page=1&amp;view={$view}">1</a>{/if}
					...
					<span class="nextBtn">{if $listing_search.current_page+1 <= $listing_search.pages_number}<a href="?searchId={$searchId}&amp;action=search&amp;page={$listing_search.current_page+1}&amp;view={$view}">[[Next]]</a>{else}<a>[[Next]]</a>{/if}
						<img src="{image}page-next.png" alt="[[Next]]"/></span>
					<span class="go-up"><a href="#">[[Go Up]]</a></span>
				</div>
			{/if}
		{else}
