					...
					<ul>
						{if $GLOBALS.user_page_uri != "/my-resume-details/" && $GLOBALS.user_page_uri != "/resume-preview/"}
							{if $acl->isAllowed('save_resume')}
								<li class="panelSavedIco"><a onclick="popUpWindow('{$GLOBALS.site_url}/saved-ads/?listing_id={$listing.id}&displayForm=1&listing_type=resume', 400, '[[Save this Resume]]', true, {if $GLOBALS.current_user.logged_in}true{else}false{/if}); return false;" href="{$GLOBALS.site_url}/saved-ads/?listing_id={$listing.id}">[[Save this Resume]]</a></li>
							{elseif $acl->getPermissionParams('save_resume') == "message"}
								<li class="panelSavedIco"><a onclick="popUpWindow('{$GLOBALS.site_url}/access-denied/?permission=save_resume', 400, '[[Save this Resume]]'); return false;" href="{$GLOBALS.site_url}/saved-ads/?listing_id={$listing.id}">[[Save this Resume]]</a></li>
							{/if}
							{if $GLOBALS.current_user.logged_in}
								{if $acl->isAllowed('save_resume')}
									<li class="panelViewDitailsIco"><a href="{$GLOBALS.site_url}/saved-resumes/">[[View Saved Resumes]]</a></li>
								{elseif $acl->getPermissionParams('save_resume') == "message"}
									<li class="panelViewDitailsIco"><a href="{$GLOBALS.site_url}/saved-resumes/" onclick="popUpWindow('{$GLOBALS.site_url}/access-denied/?permission=save_resume', 400, '[[View Saved Resumes]]'); return false;">[[View Saved Resumes]]</a></li>
								{/if}
							{else}<li class="panelViewDitailsIco"><a onclick="popUpWindow('{$GLOBALS.site_url}/saved-listings/?listing_type_id=resume', 400, '[[View Saved Resumes]]'); return false;" href="{$GLOBALS.site_url}/saved-listings/">[[View Saved Resumes]]</a></li>
							{/if}
						{/if}
						{if $acl->isAllowed('flag_resume')}
							<li class="flag-listing-ico"><a href="{$GLOBALS.site_url}/flag-listing/?listing_id={$listing.id}" onclick="popUpWindow('{$GLOBALS.site_url}/flag-listing/?listing_id={$listing.id}', 500, '[[Flag This Resume]]'); return false;">[[Flag This Resume]]</a></li>
						{elseif $acl->getPermissionParams('flag_resume') == "message"}
							<li class="flag-listing-ico"><a href="{$GLOBALS.site_url}/flag-listing/?listing_id={$listing.id}" onclick="popUpWindow('{$GLOBALS.site_url}/access-denied/?permission=flag_resume', 400, '[[Flag This Resume]]'); return false;">[[Flag This Resume]]</a></li>
						{/if}
						{if $GLOBALS.user_page_uri == "/display-resume/"}
							<li class="panelViewDitailsIco"><a href="{$GLOBALS.site_url}/display-resume/{$listing.id}?action=download_pdf_version">[[PDF Version]]</a></li>
						{else}
							<li class="panelViewDitailsIco"><a href="{$GLOBALS.site_url}/my-resume-details/{$listing.id}?action=download_pdf_version">[[PDF Version]]</a></li>
						{/if}
						{if $GLOBALS.user_page_uri == "/my-resume-details/"}
							<li class="print-listing-ico"><a target="_blank" href="{$GLOBALS.site_url}/print-my-resume/?listing_id={$listing.id}">[[Print This Ad]]</a></li>
						{else}
						...
						{if $listing.latitude && $listing.longitude && (!$GLOBALS.settings.cookieLaw || $smarty.cookies.cookiePreferences != "System")}
							<li class="viewMapIco"><a href="{$GLOBALS.site_url}/display-resume-map/?listing_id={$listing.id}&amp;searchId={$searchId}&amp;view=map" onclick="popUpWindowIframe('{$GLOBALS.site_url}/display-resume-map/?listing_id={$listing.id}&amp;searchId={$searchId}&amp;view=map&amp;lightbox=1', 810, 715, '[[Map]]'); return false;">[[Map View]]</a></li>
						{/if}
					</ul>
					<div class="clr"></div>
				</div>
				...
				<div class="comp-profile-content">
					{if $listing.anonymous != 1 || $applications.anonymous === 0 }
						...
						{if $acl->isAllowed('use_private_messages')}
							<br /><span class="link"><a href="{$GLOBALS.site_url}/private-messages/contacts/?pm_action=save_contact&amp;user_id={$listing.user.id}" onclick="popUpWindow('{$GLOBALS.site_url}/private-messages/contacts/?pm_action=save_contact&amp;user_id={$listing.user.id}', {if $GLOBALS.current_user.logged_in}400{else}500{/if}, '[[Save Contact]]', true, {if $GLOBALS.current_user.logged_in}true{else}false{/if}); return false;"  class="pmSendLink">[[Save Contact]]</a></span>
						{elseif $acl->getPermissionParams('use_private_messages') == "message"}
							<br /><span class="link"><a href="#" onclick="popUpWindow('{$GLOBALS.site_url}/access-denied/?permission=use_private_messages', 400, '<span>[[Apply Now]]</span>: {$listing.Title|escape:"html"}'); return false;"  class="pmSendLink">[[Save Contact]]</a></span>
						{/if}
						{if $listing.Resume.file_url != ""}
							<br /><span class="link"><a href="?filename={$listing.Resume.saved_file_name|escape:'url'}">[[Download Resume]]</a></span>
						{/if}
						{if $acl->isAllowed('view_resume_search_results')}
							<br /><span class="link"><a href="{$GLOBALS.site_url}/search-results-resumes/?action=search&amp;username[equal]={$listing.user.id}">[[All resumes by this user]]</a></span>
						{/if}
						{if $acl->isAllowed('use_private_messages')}
							<br /><span class="link"><a href="{$GLOBALS.site_url}/private-messages/send/?to={$listing.user.id}" onclick="popUpWindow('{$GLOBALS.site_url}/private-messages/aj-send/?to={$listing.user.id}', 700, '[[Send private message]]', true, {if $GLOBALS.current_user.logged_in}true{else}false{/if}); return false;"  class="pm_send_link">[[Send private message]]</a></span>
						{elseif $acl->getPermissionParams('use_private_messages') == "message"}
							<br /><span class="link"><a href="{$GLOBALS.site_url}/private-messages/send/?to={$listing.user.id}" onclick="popUpWindow('{$GLOBALS.site_url}/access-denied/?permission=use_private_messages', 400, '[[Send private message]]'); return false;"  class="pm_send_link">[[Send private message]]</a></span>
						{/if}
					{else}
						...
						{/if}
						{if $acl->isAllowed('use_private_messages')}
							<br /><span class="strong"><span class="link"><a href="{$GLOBALS.site_url}/private-messages/send/?to={$listing.user.id}&amp;anonym=1" onclick="popUpWindow('{$GLOBALS.site_url}/private-messages/aj-send/?to={$listing.user.id}&anonym=1', 600, '[[Send private message]]', true, {if $GLOBALS.current_user.logged_in}true{else}false{/if}); return false;"  class="pm_send_link">[[Send private message]]</a></span></span>
						{elseif $acl->getPermissionParams('use_private_messages') == "message"}
							<br /><span class="strong"><span class="link"><a href="{$GLOBALS.site_url}/private-messages/send/?to={$listing.user.id}&amp;anonym=1" onclick="popUpWindow('{$GLOBALS.site_url}/access-denied/?permission=use_private_messages', 400, '[[Send private message]]'); return false;"  class="pm_send_link">[[Send private message]]</a></span</span>
						{/if}
					{/if}
				</div>
	...
	<div class="after-quick-links">
		<ul>
			...
			{if $GLOBALS.user_page_uri == "/my-resume-details/"}
				<li class="panelViewDitailsIco"><a href="{$GLOBALS.site_url}/display-resume/{$listing.id}?action=download_pdf_version">[[PDF Version]]</a></li>
			{else}
				<li class="panelViewDitailsIco"><a href="{$GLOBALS.site_url}{$url}?action=download_pdf_version">[[PDF Version]]</a></li>
			{/if}
			{if $acl->isAllowed('flag_resume')}
				<li class="flag-listing-ico"><a href="{$GLOBALS.site_url}/flag-listing/?listing_id={$listing.id}" onclick="popUpWindow('{$GLOBALS.site_url}/flag-listing/?listing_id={$listing.id}', 500, '[[Flag This Resume]]'); return false;">[[Flag This Resume]]</a></li>
			{elseif $acl->getPermissionParams('flag_resume') == "message"}
