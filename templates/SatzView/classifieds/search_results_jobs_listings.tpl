		...
		<tr {if $listing.priority == 1}class="priorityListing"{else}class="{cycle values = 'evenrow,oddrow' advance=true}"{/if}>
			<td>
				<div class="listing-section {if $show_brief_or_detailed == 'brief'}brief-view{/if}">
					...
					<div class="listing-info">
						<div class="left-side">
							<a href="{$GLOBALS.site_url}/company/{$listing.user.id}/{$listing.user.CompanyName|regex_replace:"/[\s\/\\\\]/":"-"|escape:"url"}/">{$listing.user.CompanyName}</a>
							{locationFormat location=$listing.Location format="short"}
							<div class="show-brief">
						...
						<div class="right-side">
							{if $listing.user.Logo.file_url}
								{if $show_brief_or_detailed != 'brief'}
									<a href="{$GLOBALS.site_url}/company/{$listing.user.id}/{$listing.user.CompanyName|regex_replace:"/[\s\/\\\\]/":"-"|escape:"url"}/"><img src="{$listing.user.Logo.file_url}" alt="" /></a><br/>
								{/if}
							{/if}
							<span>[[Posted]]: [[$listing.activation_date]]</span>
						</div>
