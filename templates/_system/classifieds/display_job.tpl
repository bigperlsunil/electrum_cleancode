		...
		<div class="userInfo">
			...
			<div class="compProfileInfo">
				{if $listing.anonymous != 1 || $applications.anonymous === 0 }
					...
					{if $listing.user.isJobg8}
						<a href="{$GLOBALS.site_url}/company/{$listing.user.id}/{$listing.user.CompanyName|regex_replace:"/[\s\/\\\\]/u":"-"|escape:"url"}/">[[Company Profile]]</a>
					{else}
						...
						{/if}
						<a href="{$GLOBALS.site_url}/company/{$listing.user.id}/{$listing.user.CompanyName|regex_replace:"/[\s\/\\\\]/u":"-"|escape:"url"}/">[[Company Profile]]</a><br/>
					{/if}
				...
				{if $acl->isAllowed('apply_for_a_job')}
					...
					{else}
						{if isset($listing.ApplicationSettings.add_parameter) && $listing.ApplicationSettings.add_parameter == 2}
							{capture assign='applyBtn_onClick'}onclick="javascript:window.open('{if $listing.user.isJobg8 && $listing.jobType == 'APPLICATION'}{$GLOBALS.site_url}/apply-now-external/?listing_id={$listing.id}{else}{$listing.ApplicationSettings.value}{/if}');"{/capture}
						{else}
