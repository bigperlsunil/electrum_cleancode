...
{foreach from=$listings item=listing name=listings}
	{if $listing.api}
		{if $api != $listing.api}
			...
			{if $api == 'indeed'}
				{assign var='indeedPageNavigation' value=1}
			...
			<td colspan="6">
				<table>
					<tr>
						...
						<td width="50%">
							<a name="listing_{$listing.id}"></a>
							<a href="{$GLOBALS.site_url}/display-job/{$listing.id}/{$listing.Title|regex_replace:"/[\\/\\\:*?\"<>|%#$\s]/u":"-"}.html?searchId={$searchId}&amp;page={$listing_search.current_page}"><span class="strong">{$listing.Title|escape:'html'}</span></a>
						</td>
						<td width="20%">
						...
						<td colspan="4">
							<ul>
								...
								<li class="viewDetails">
									<a href="{$GLOBALS.site_url}/display-job/{$listing.id}/{$listing.Title|regex_replace:"/[\\/\\\:*?\"<>|%#$\s]/u":"-"}.html?searchId={$searchId}&amp;page={$listing_search.current_page}">[[View job details]]</a>
								</li>
...
{* Page navigation for Indeed search results *}
{if $api == 'indeed' && !$indeedPageNavigation}
	<tr class="indeedBlock">
		<td colspan="6" style="text-align: right;">
			<span id="indeed_ajax_preloader_listings_results" class="preloader">&nbsp;<img src="{$GLOBALS.site_url}/templates/_system/main/images/ajax_preloader_circular_16.gif" />&nbsp;</span>
			{if $currentIndeedPage > 1}<a onclick="requestToListingsProviders({$currentIndeedPage-1}, 'indeed');return false;" href="#" style="text-decoration: none;">&lt;&lt;</a>{/if}
