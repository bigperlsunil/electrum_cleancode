	...
	<table cellspacing="0" id="my-listings-table">
		...
		<tbody>
			...
			<tr {if $listing.priority == 1}class="priorityListing"{else}class="{cycle values = 'evenrow,oddrow' advance=false}"{/if}>
				...
				<td><a href="{$GLOBALS.site_url}/{$link}/{$listing.id}/{$listing.Title|regex_replace:"/[\\/\\\:*?\"<>|%#$\s]/u":"-"}.html"><span class="strong">{$listing.Title} {if $listing.anonymous == 1}(anonymous){/if}</span></a></td>
				<td>
					{if $hasSubusersWithListings}
			...
			<tr {if $listing.priority == 1}class="priorityListing"{else}class="{cycle values = 'evenrow,oddrow'}"{/if}>
				...
				<td colspan="9">
					<ul>
						<li>&nbsp;<a href="{$GLOBALS.site_url}/{$link}/{$listing.id}/{$listing.Title|regex_replace:"/[\\/\\\:*?\"<>|%#$\s]/u":"-"}.html">[[View details]]</a> |</li>
