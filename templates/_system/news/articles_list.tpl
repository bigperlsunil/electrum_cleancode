...
{else}
	...
	{foreach from=$articles item=item}
		<div class="newsItems">
			...
			{else}
				<h2><a href="{$GLOBALS.site_url}/news/{$item.sid}/{$item.title|regex_replace:"/[\\/\\\:*?\"<>|%#$\s]/u":"-"}.html">{$item.title}</a></h2>
			{/if}
			<div class="newsPreview">
				...
				{else}
					<p align="right"><a href="{$GLOBALS.site_url}/news/{$item.sid}/{$item.title|regex_replace:"/[\\/\\\:*?\"<>|%#$\s]/u":"-"}.html">[[read more]]</a></p>
				{/if}
				<div class="clr"></div>
			</div>
		</div>
