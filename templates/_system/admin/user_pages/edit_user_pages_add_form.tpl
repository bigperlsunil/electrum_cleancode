...
{/if}
    ...
    {foreach from=$ERRORS key=ERROR item=ERROR_DATA}
    	...
    	{if $ERROR == 'ACCESS_DENIED' && $ERROR_DATA == 'demo'}<p class="error">{tr}You don't have permissions for it. This is a Demo version of the software.{/tr}</p>{/if}
    	{if $ERROR == 'ACCESS_DENIED' && $ERROR_DATA == 'trial'}<p class="error">{tr}You don't have permissions for it. This is a Trial version of the software.{/tr}</p>{/if}
    {/foreach}
