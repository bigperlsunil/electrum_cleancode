...
{foreach from=$ERRORS key=error item=errorData}
	{if $error == 'ACCESS_DENIED' && $errorData == 'demo'}<p class="error">[[You don't have permissions for it. This is a Demo version of the software.]]</p>{/if}
	{if $error == 'ACCESS_DENIED' && $errorData == 'trial'}<p class="error">[[You don't have permissions for it. This is a Trial version of the software.]]</p>{/if}
{/foreach}
<table>
	<thead>
		<tr>
