					...
					<tbody>
						{foreach from=$listings item=listing name=listings_block}
							<tr class="{cycle values = 'evenrow,oddrow'}">
								...
								<td><a href="{$GLOBALS.site_url}/edit-listing/?listing_id={$listing.id}">{$listing.id}</a></td>
								<td><a href="{$GLOBALS.site_url}/edit-listing/?listing_id={$listing.id}">{$listing.Title|escape:'html'}</a></td>
								<td>[[{$listing.product.name}]]</td>
								<td><span title="{$listing.activation_date}">{$listing.activation_date|regex_replace:"/\s.*/u":""}</span></td>
								<td><span title="{$listing.expiration_date}">{$listing.expiration_date|regex_replace:"/\s.*/u":""}</span></td>
								<td>
