	...
	<div id="settingsPane">
        ...
        <div id="performanceTab" class="ui-tabs-panel">
            ...
            <table class="basetable" width="100%">
				...
				<tr class="{cycle values = 'evenrow,oddrow'}">
					<td>[[Clean up statistics after]]</td>
					<td>
						<input type="text" name="statistics_lifetime" style="width: 30px" value="{$settings.statistics_lifetime}" /> [[months]]
					</td>
				</tr>
        ...
        <div id="notificationsTab" class="ui-tabs-panel ui-tabs-hide">
			...
			<table class="basetable" width="100%">
				...
				<tr class="{cycle values = 'evenrow,oddrow'}">
					<td>[[Notify Sub-Admin on Account Registration]]</td>
					<td>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('[type="checkbox"][name="use_autocomplete_for_keywords"]').change(function(e) {
			var self = $(this);
			if (!self.is(':checked')) {
				$("#messageBox").dialog( 'destroy' ).html('{capture name="disableAutocompleteMessage"}[[You are now going to disable autocomplete for keyword search. This will also remove all keywords collected for this function.]]{/capture}{$smarty.capture.disableAutocompleteMessage|escape:'quotes'}');
				$("#messageBox").dialog({
					width: 500,
					height: 150,
					title: '[[Attention!]]',
