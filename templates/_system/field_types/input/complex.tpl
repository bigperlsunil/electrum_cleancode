...
<script type="text/javascript">
	...
	function addComplexField_{$complexField}() {ldelim}
		...
		newField.find('input[type="file"]').each(function(){ldelim}
			if ($(this).attr('field_id')){ldelim}
				...
				if (fileFieldContent) {ldelim}
					...
					fileFieldContent.children('input[id^="hidden_"]').remove();
					fileFieldContent.append('<input type="hidden" id="hidden_' + $(this).attr( 'name' ) + '" name="' + $(this).attr( 'name' ) + '" value="" class="complexField"/>');
				{rdelim}
			{rdelim}
		{rdelim});
		...
		$("#" + id + " select").val("");
		$("#" + id + " select.selectCurrency").val({$GLOBALS.default_currency.sid});
