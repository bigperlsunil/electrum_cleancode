...
class SJB_Error
{
	...
	public static function writeToLog($errors)
	{
		...
		// clear log
		if (rand(1, 100) == 100) {
			$id = SJB_DB::queryValue('SELECT `sid` FROM `error_log` ORDER BY `sid` DESC LIMIT 100, 1');
			if ($id) {
				SJB_DB::query('DELETE FROM `error_log` WHERE `sid` <= ?n', $id);
			}
		}
	...
	{
		return SJB_DB::query('SELECT `date`, `errors` FROM `error_log` ORDER BY `sid` DESC LIMIT ?n', $recordsNum);
	}
