			...
			switch ($type) {
				...
				case 'guest':
					if ($roleId == 'guest' || $type == 'guest') {
						...
						if (intval($userInfo['parent_sid']) > 0 && strpos($resource, 'subuser_') === false) {
							$permissions = $this->getPermissions('user', $roleId);
							$parentPermissions = $this->getPermissions('user', $userInfo['parent_sid']);
							$permissions = array_merge($parentPermissions, $permissions);
							$roleId = $userInfo['parent_sid'];
						} else {
							$permissions = $this->getPermissions('user', $roleId);
						}
