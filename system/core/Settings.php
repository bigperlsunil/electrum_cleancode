	...
	public static function updateSettings($settings)
	{
		...
		foreach ($settings as $name => $value) {
			if ($name == 'disable_bots') {
				...
				$rebuildBrowses = $name == 'enableBrowseByCounter' && $value && !self::getValue('enableBrowseByCounter');
				$truncateKeywords = $name == 'use_autocomplete_for_keywords' && !$value && self::getValue('use_autocomplete_for_keywords');
				if ($rebuildBrowses) {
					SJB_BrowseDBManager::rebuildBrowses();
				}
				if ($truncateKeywords) {
					SJB_DB::query('TRUNCATE `listings_keywords`');
				}
