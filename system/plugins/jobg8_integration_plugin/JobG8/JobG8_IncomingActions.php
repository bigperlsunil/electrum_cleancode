...
class JobG8_IncomingActions extends JobG8
{
	...
	private function getUserSIDByCompanyName($companyName)
	{
		...
		if (!empty($user['username'])) {
			...
			if (empty($userSID)) {
				$userObj = SJB_ObjectMother::createUser($user, $userGroupSid);
				$userObj->deleteProperty('active');
				$userObj->deleteProperty('featured');
				$userObj->deleteProperty('captcha');
				$userObj->addParentProperty($currentUserInfo['sid']);
	...
	private function isAllowedToCreateThisJob($contractsSIDs, $job)
	{
		...
		$ignoreFields = array(
			...
			$mapper->salaryMappingFieldID     => array('id' => '', 'name' => '')
		);
		...
		if (isset($job->PayPeriod)) {
			$ignoreFields[$mapper->salaryMappingFieldID] = array(
				'id'   => (int) $job->PayPeriod['ValueID'],
				'name' => (string) $job->PayPeriod
			);
		}
	...
	private function getSalary($job)
	{
		if (isset($job->PayAmount)) {
			$salary = (string) $job->PayAmount;
		} else if(isset($job->PayMaximum)) {
			$salary = (string) $job->PayMaximum;
		} else {
			$salary = '';
		}
		if (strstr($salary, '.') !== false) {
			$salary = floor($salary);
		}
		$currencyCode = '';
		...
		return array('value' => $salary, 'add_parameter' => $currentCurrency['sid']);
	}
	...
	private function getListingData($listing, $job, $contractID)
	{
		...
		$locationFields = array(
			'Country' 	=> isset($job->Country) ? SJB_CountriesManager::getCountrySIDByCountryName((string) $job->Country) : (isset($location['Country']) ? $location['Country'] : ''),
			'ZipCode' 	=> isset($job->PostCode) && SJB_LocationManager::doesLocationExist((string) $job->PostCode) ? (string) $job->PostCode : (isset($location['ZipCode']) ? $location['ZipCode'] : '')
		);
		if (isset($job->Country) && (int)$job->Country['ValueID'] == 246) { // United Kingdom
			$cityAndState = array(
				'State' 	=> '',
				'City'		=> isset($job->Location) ? (string) $job->Location == 'Not Specified' ? '' : (string) $job->Location : (isset($location['City']) ? $location['City'] : ''),
			);
		} else {
			$cityAndState = array(
				'State' 	=> isset($job->Location) ? SJB_StatesManager::getStateSIDByStateName((string) $job->Location) : (isset($location['State']) ? $location['State'] : ''),
		...
		$listingData = array(
			...
			'Salary'		 		=> isset($job->PayAmount) || isset($job->PayMaximum) ? $this->getSalary($job) : $listing->getPropertyValue('Salary'),
			'SalaryType'     		=> isset($job->PayPeriod) ? $this->mapper->getSalaryTypeMappingValue((int) $job->PayPeriod['ValueID']) : $listing->getPropertyValue('SalaryType'),
	...
	private function createListing($listingData, $listingTypeSid, $contractID, $activate = false, $userSID = null)
	{
		...
		SJB_ListingManager::setListingApprovalStatus($listing->getSID(), 'approved', false);
		if ($activate) {
		...
		return '
			<q1:PostAdverts xmlns:q1="urn:jobg8">
				...
				<PostAdvert xmlns="http://jobg8.com/">
					...
					<LogoURL>http://www.smartjobboard.com/templates/SmartJobBoard/main/images/smartjobboard-logo.png</LogoURL>
