...
class JobG8_OutgoingActions extends JobG8
{
	...
	private function getElements($action, $listingSID)
	{
		if ($action == 'delete') {
			...
			$userEmail = $listingStructure['user']['email'];
			...
			$currency = isset($listingStructure['Salary']['add_parameter']) ? SJB_CurrencyManager::getCurrencyBySID($listingStructure['Salary']['add_parameter']) : '';
			$currencyCode = '';
			if (!empty($currency)) {
				$currencyCode = isset($currency['currency_code']) ? $currency['currency_code'] : '';
			}
			...
			$elements = array(
				...
				'url'            => SJB_System::getSystemSettings('SITE_URL') . "/display-job/" . $listingStructure['id'],
				...
				'salaryAmount'   => isset($listingStructure['Salary']['value']) ? $listingStructure['Salary']['value'] : '',
				'salaryCurrency' => $currencyCode,
				'salaryType'     => $listingStructure['SalaryType'],
				...
				'video'          => $listingStructure['video']['file_url'],
