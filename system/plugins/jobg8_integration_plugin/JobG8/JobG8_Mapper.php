...
class JobG8_Mapper 
{
	...
	private $jobg8SalaryTypes = array(
		2175 => "Hourly",
		7838 => "Daily",
		2176 => "Weekly",
		2177 => "Monthly",
	...
	private $salaryTypeMappingValues = array(
		2175 => "per hour",
		7838 => "per day",
		2176 => "per week",
		2177 => "per month",
	...
	public function __construct()
	{
		...
		$salaryMappingFieldID = SJB_Settings::getSettingByName('salaryMappingFieldID');
		$this->salaryMappingFieldID = empty($salaryMappingFieldID) ? $this->salaryMappingFieldID : $salaryMappingFieldID;
	}
	...
	/**
	 * @param int $jobg8SalaryTypeSID
	 * @return array
	 */
	public function getSalaryTypeMappingValue($jobg8SalaryTypeSID)
	{
		if (empty($jobg8SalaryTypeSID) || !isset($this->jobg8SalaryTypes[$jobg8SalaryTypeSID])) {
			return '';
		}
		$jobg8SalaryTypeValue = $this->jobg8SalaryTypes[$jobg8SalaryTypeSID];
		$mappedValue = $this->getSJBFieldValueByJobg8FieldValue($jobg8SalaryTypeValue);
		if (empty($mappedValue)) {
			$fieldSid = SJB_ListingFieldManager::getListingFieldSIDByID($this->salaryMappingFieldID);
			$itemSid = SJB_ListingFieldManager::getListItemSIDByValue($jobg8SalaryTypeValue, $fieldSid);
			$mappedValue = $itemSid;
		}
	 ...
	 * @return string
	 */
	public function getValueOfTreeFieldForItemName($fieldId, $itemNames)
	{
		if (empty($fieldId) || empty($itemNames)) {
			return '';
		}
		$mappedValue = '';
		$fieldSid = SJB_ListingFieldManager::getListingFieldSIDByID($fieldId);
		foreach ($itemNames as $itemName) {
			$itemName = html_entity_decode($itemName);
			$itemInfo  = SJB_ListingFieldTreeManager::getItemInfoByCaption($fieldSid, $itemName);
			if (!empty($itemInfo['sid'])) {
				$childrenSIDs = SJB_ListingFieldTreeManager::getChildrenSIDBySID($itemInfo['sid']);
				$mappedValue = empty($childrenSIDs) ? $itemInfo['sid'] : implode(',', $childrenSIDs);
				break;
			}
		}
	...
	public function setDefaultSalaryMapping()
	{
		foreach ($this->salaryTypeMappingValues as $jobg8SalaryTypeSID => $sjbSalaryTypeMappingValue) {
			$fieldSid = SJB_ListingFieldManager::getListingFieldSIDByID($this->salaryMappingFieldID);
			$mappedValue = '';
			$itemSid = SJB_ListingFieldManager::getListItemSIDByValue($sjbSalaryTypeMappingValue, $fieldSid);
			if (!empty($itemSid)) {
				$mappedValue = $itemSid;
			}
			SJB_DB::queryExec('UPDATE ?w SET `sjb_field_value` = ?s WHERE `sid` = ?n', $this->tableName, $mappedValue, $jobg8SalaryTypeSID);
		}
	}
	...
	public function setSalaryMappingType()
	{
		if (!$this->isExistMappingType(self::SALARY_MAPPING_TYPE)) {
			$mapper = new JobG8_Mapper();
			foreach ($mapper->jobg8SalaryTypes as $jobg8SalaryTypeSID => $jobg8SalaryTypeValue) {
				SJB_DB::queryExec('INSERT INTO ?w SET `sid` = ?n, `type` = ?s, `jobg8_field_value` = ?s', $this->tableName, $jobg8SalaryTypeSID, self::SALARY_MAPPING_TYPE, $jobg8SalaryTypeValue);
			}
		}
	}
		...
		if (SJB_ListingFieldManager::getListingFieldSIDByID($mappingField)) {
			...
			switch ($mappingType) {
				...
				case self::EMPLOYMENT_MAPPING_TYPE: $this->employmentMappingFieldID = $mappingField;
					break;
				case self::SALARY_MAPPING_TYPE: $this->salaryMappingFieldID = $mappingField;
					break;
			}
		} else {
	...
	public function isAllFieldMappedByType($mappedFieldValues, $mappingType, $allowedMappingFields)
	{
		...
		switch ($mappingType) {
			...
			case self::EMPLOYMENT_MAPPING_TYPE: $jobg8MappingFields = $this->jobg8EmploymentTypes;
				break;
			case self::SALARY_MAPPING_TYPE: $jobg8MappingFields = $this->jobg8SalaryTypes;
				break;
		}
