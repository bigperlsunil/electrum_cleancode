...
<form method="post" class="mapping">
	...
	<div class="clr"><br/></div>
	<div class="setting_button" id="mediumButton">[[Click for salary types mapping]]<div class="setting_icon"><div id="accordeonClosed"></div></div></div>
	<div class="setting_block" style="display: none" id="clearTable">
		<table id="salary" class="mapping-salary" width="100%">
			<thead>
				<tr>
					<th>[[JobG8 salary types]]</th>
					<th>
						[[SJB field]]: 
						<input type="text" class="mappingField" value="{$salaryMappingFieldID}" />
						<input type="submit" class="grayButton"  value="{if $salaryMappingFieldID}[[Change Field]]{else}[[Set Field]]{/if}" />
					</th>
				</tr>
			</thead>
			<tbody>
				{foreach from=$salaryMappingFieldValues item="salaryMappingFieldValue"}
					<tr class="{cycle values = 'evenrow,oddrow'}">
						<td valign="top">
							<input type="hidden" name="allow[{$salaryMappingFieldValue.sid}]" value="0" />
							<input type="checkbox" name="allow[{$salaryMappingFieldValue.sid}]" value="1" {if $salaryMappingFieldValue.allow}checked="checked"{/if} />
							[[{$salaryMappingFieldValue.jobg8_field_value}]]
						</td>
						<td valign="top">
							<input type="hidden" name="salary[{$salaryMappingFieldValue.sid}]" value="" />
							<select>
								<option value=""></option>
								{foreach from=$sjbSalaryTypes item="sjbSalaryType"}
									<option value="{$sjbSalaryType.id}" {if $sjbSalaryType.id == $salaryMappingFieldValue.sjb_field_value}selected="selected"{/if}>{tr}{$sjbSalaryType.caption}{/tr|escape:'html'}</option>
								{/foreach}
							</select>
						</td>
					</tr>
