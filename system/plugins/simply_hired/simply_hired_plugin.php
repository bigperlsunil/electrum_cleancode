...
class SimplyHiredPlugin extends SJB_PluginAbstract 
{
	...
	public static function getListingsFromSimplyHired($params)
	{
		...
		if ($listingTypeID == 'Job' && $GLOBALS['uri'] == '/search-results-jobs/' || $GLOBALS['uri'] == '/ajax/') {
			...
			$apiKey = SJB_Settings::getSettingByName('simplyHiredAPIKey');
			...
			$url = "http://api.{$siteUrl}/a/jobs-api/xml-v2/q-{$keywords}/l-{$location}/mi-$radius/ws-$limit/pn-{$params->listing_search_structure['current_page']}/sb-{$sortBy}?pshid={$publisherID}&ssty={$ssty}&cflg=r&auth={$apiKey}&clip={$ip}";
			$sxml = new simplexml();
					...
					foreach ($results as $node) {
						...
						foreach ($node as $key => $result) {
							...
							$simplyhiredListings [$key] = array(
								...
								'code'			 => '<a style="text-decoration:none" href="//www.simplyhired.com/" rel="nofollow"><img src="//www.simply-partner.com/static/_/img/sh-logo-wide.png" alt="jobs by simply hired" title="simply hired job feed"></a>'
							);
						}
					}
				}
