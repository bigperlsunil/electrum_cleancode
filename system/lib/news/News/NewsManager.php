...
class SJB_NewsManager extends SJB_ObjectManager
{
	...
	public static function getNewsByPage($page = 1, $itemsPerPage = 10, $categorySid = null, $lang = 'en', $active = false)
	{
		...
		$activeParam = '';
		if ($active) {
			$activeParam = 'AND `active` = 1 AND (`date` < NOW() OR `date` IS NULL)';
		}
		if (empty($categorySid)) {
	 ...
	 * @return boolean
	 */
	public static function checkExistsCategoryName($name, $categoryID = false)
	{
		if ($categoryID) {
			$result = SJB_DB::query("SELECT * FROM `news_categories` WHERE `name` = ?s AND `id` != ?n LIMIT 1", $name, $categoryID);
		} else {
