...
class SJB_XmlImport
{
	...
	public static function saveImport($parserInfo, $addNewUser = 0) {
		...
		if (!empty($parserInfo['logo_options']) && $parserInfo['logo_options'] != 'not_logo'){
			...
			if (!SJB_ListingFieldDBManager::getListingFieldInfoByID('ListingLogo')) {
				...
				$listing_field = new SJB_ListingField($listing_field_info, $parserInfo['type_id']);
		...
		$errorFile = '';
		$xml_logo = !empty($parserInfo['xml_logo']) ? $parserInfo['xml_logo'] : '';
		$selectedLogoOption = !empty($parserInfo['logo_options']) ? $parserInfo['logo_options'] : '';
		$selectedLogoField = !empty($parserInfo['import_logo_field']) ? $parserInfo['import_logo_field'] : '';
			...
			case 'upload_logo' :
				if ( ! empty($_FILES['upload_logo_file'])) {
					if ($_FILES['upload_logo_file']['error']) {
						$errorFile['Logo Options'] = SJB_UploadFileManager::getErrorId($_FILES['upload_logo_file']['error']);
					} else {
		...
		if (empty($errorFile)) {
			...
			if ($parserInfo['id'] > 0) {
				SJB_DB::query("UPDATE `parsers` {$query} WHERE id = ?n", $parserInfo['custom_script_users'], $parserInfo['custom_script'], $parserInfo['type_id'], $parserInfo['parser_name'], $parserInfo['form_description'], $parserInfo['parser_url'], $parserInfo['usr_id'], $parserInfo['usr_name'], $serUserMap, $parserInfo['xml'], $addNewUser, $parserInfo['username'], $parserInfo['external_id'], $parserInfo['postUnderProduct'], $parserInfo['import_type'], $parserInfo['default_value'], $parserInfo['user_default_value'], $serMap, $xml_logo, $logo_options, $queryId);
			} else {
				$parserInfo['id'] = SJB_DB::query("INSERT INTO `parsers` {$query}", $parserInfo['custom_script_users'], $parserInfo['custom_script'], $parserInfo['type_id'], $parserInfo['parser_name'], $parserInfo['form_description'], $parserInfo['parser_url'], $parserInfo['usr_id'], $parserInfo['usr_name'], $serUserMap, $parserInfo['xml'], $addNewUser, $parserInfo['username'], $parserInfo['external_id'], $parserInfo['postUnderProduct'], $parserInfo['import_type'], $parserInfo['default_value'], $parserInfo['user_default_value'], $serMap, $xml_logo, $logo_options);
			}
		}
		return array('id' => $parserInfo['id'], 'errors' => $errorFile);
	}
	...
	public static function addListings($data, $usr_id, $parser_id, $script)
	{
		...
		foreach ($data as $listing) {
			...
			if ($logo_options['option'] == 'upload_logo') {
			...
			else if ($logo_options['option'] == 'import_logo' && !empty($listing['ListingLogo']) && filter_var($listing['ListingLogo'], FILTER_VALIDATE_URL)) {
				...
				$headers = @get_headers($listing['ListingLogo']);
				if (!empty($headers) && strpos('200', array_shift($headers))) {
			...
			if (!$updatedListing) {
				...
				if (!empty($listing['expiration_date'])) {
					if (SJB_ListingDBManager::validDate($listing['expiration_date'])) {
						SJB_ListingDBManager::setListingExpirationDate($listingSid, $listing['expiration_date']);
					}
				}
