...
class SJB_FormBuilderManager
{
	...
	public static function deleteListingSubFieldBySidFromFieldsHolder($fieldSID, $subFieldID)
	{
		return SJB_DB::query('UPDATE `formbuilders_fieldsholders` SET `html` = REPLACE(`html`, ?s, ?s) WHERE `field_sid` = ?s', '{'.$subFieldID.'}', '', $fieldSID);
	}
