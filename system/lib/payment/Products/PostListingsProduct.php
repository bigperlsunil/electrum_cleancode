...
class SJB_PostListingsProduct extends SJB_ProductDetails 
{
	...
	public function saveProduct($product, $request = array())
	{
		...
		switch ($pricingType) {
			case 'volume_based':
				...
				$objectProperties = $product->getProperties();
				$volumeBasedPricingProperty = $objectProperties['volume_based_pricing'];
				$fields = $volumeBasedPricingProperty->type->complex;
				$fields->setPropertyValue('listings_range_from', $property['listings_range_from']);
				$fields->setPropertyValue('listings_range_to', $property['listings_range_to']);
				$fields->setPropertyValue('price_per_unit', $property['price_per_unit']);
				$fields->setPropertyValue('renewal_price_per_listing', $property['renewal_price_per_listing']);
				$volumeBasedPricingProperty->type->complex = $fields;
				$product->details->properties['volume_based_pricing'] = $volumeBasedPricingProperty;
	...
	public function isValid($product)
	{
		...
		if ($pricingType == 'volume_based') {
			$volumeBasedPricing = $product->getPropertyValue('volume_based_pricing');
			if (!empty($volumeBasedPricing['listings_range_from'])) {
				foreach ($volumeBasedPricing['listings_range_from'] as $i => $listingsRangeFrom) {
