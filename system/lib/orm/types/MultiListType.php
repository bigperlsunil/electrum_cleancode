...
class SJB_MultiListType extends SJB_Type
{
	...
	function getPropertyVariablesToAssign()
	{
		...
		if (!empty($propertyVariables['profile_field_as_dv'])) {
			$profile_field_as_dv = explode(',', $propertyVariables['profile_field_as_dv']);
			foreach ($profile_field_as_dv as $key => $itemSID) {
				$profile_field_as_dv[$key] = SJB_ListingFieldManager::getListItemSIDByValue(SJB_UserProfileFieldManager::getListItemValueBySID($itemSID), $this->property_info['sid']);
			}
			$profile_field_as_dv = implode(',', $profile_field_as_dv);
		}
		$value = array(
