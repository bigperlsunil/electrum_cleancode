...
class SJB_ComplexFileType extends SJB_Type
{
	...
	function isEmpty() 
	{
		if ($this->getComplexParent()) {
			return parent::isEmpty() && !SJB_UploadFileManager::isFileReadyForUpload($this->property_info['id'], $this->getComplexParent(), $this->complexEnum);
		}
	...
	function isValid()
	{
		...
		if (!isset($_FILES[$this->getComplexParent()]['name'][$this->fieldID][$this->complexEnum]) || $_FILES[$this->getComplexParent()]['name'][$this->fieldID][$this->complexEnum] == '') {
			return true;
		}
		$upload_manager = new SJB_UploadFileManager();
		...
		if ($this->getComplexParent()) {
			if ($upload_manager->isValidUploadedFile($this->getComplexParent(), $this->property_info['id'], array(), $this->complexEnum)) {
				return true;
			}
				...
				if (file_exists($fileUrl)) {
					switch ($fileExtension) {
						case 'doc':
							$doc = new doc();
							$doc->read($fileUrl);
							$keywords = preg_replace('/[\n\r]/u', '', strip_tags($doc->parse()));
							break;
						case 'docx':
							$keywords = SJB_HelperFunctions::docx2text($fileUrl);
							$keywords = preg_replace('/[\n\r]/u', '', strip_tags(html_entity_decode($keywords)));
							break;
						case 'xls':
						case 'xlsx':
