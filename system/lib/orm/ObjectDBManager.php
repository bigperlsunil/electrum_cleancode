...
class SJB_ObjectDBManager
{
	...
	public static function saveObject($db_table_name, SJB_Object $object, $sid = false, $listingSidsForCopy = array())
	{
		...
		if (!empty($listingSidsForCopy)) {
			if (!empty($listingSidsForCopy['copyFileFields'])) {
				SJB_ListingManager::copyFilesAndPicturesFromListing($listingSidsForCopy['filesFrom'], $object_sid, $listingSidsForCopy['picturesFrom'], $listingSidsForCopy['copyFileFields']);
			} else {
