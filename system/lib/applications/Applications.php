...
class SJB_Applications
{
	...
	public static function getByJob($listingID, $orderInfo = false, $score = false, $limit = false)
	{
		...
		FROM `applications` `a`
			INNER JOIN `listings` l ON
				...
				{$scoreFilter['inner_join']}
				{$order['join']}
				WHERE `a`.`listing_id` = ?s AND`a`.`show_emp` = 1 {$scoreFilter['case']} {$order['order']} {$limitFilter}", $listingID);
		return $apps;
	}
	...
	public static function getCountAppsByJob($listingID, $score = false)
	{
		...
		INNER JOIN `listings` l ON
			`l`.`sid` = `a`.`listing_id`
			{$scoreFilter['inner_join']}
		WHERE `a`.`listing_id` = ?s AND `a`.`show_emp` = 1 {$scoreFilter['case']}", $listingID);
	...
	public static function getByEmployer($userSID, $orderInfo, $score = false, $subuser = false, $limit = false)
	{
		...
		$apps = SJB_DB::query("
			...
			INNER JOIN `listings` l ON
				...
				{$scoreFilter['inner_join']}
				{$order['join']}
			WHERE `l`.`user_sid` = ?s AND `a`.`show_emp` = 1 {$scoreFilter['case']} {$subuserFilter} {$order['order']} {$limitFilter}", $userSID);
		return $apps;
	}
	...
	public static function getCountApplicationsByEmployer($userSID, $score = false, $subuser = false)
	{
		...
		$appsCount = SJB_DB::queryValue("
			...
			INNER JOIN `listings` l ON
				`l`.`sid` = `a`.`listing_id`
				{$scoreFilter['inner_join']}
			WHERE `l`.`user_sid` = ?s AND `a`.`show_emp` = 1 {$scoreFilter['case']}{$subuserFilter} ", $userSID);
		return $appsCount;
	}
