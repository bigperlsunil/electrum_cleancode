...
class SJB_LocationCriterion extends SJB_SearchCriterion
{
	function getSystemSQL($table = '')
	{
		...
		$selectFields = "`name`, `city`, `state_code`";
		if (!empty($this->value['radius']) && is_numeric($this->value['radius'])) {
			$selectFields = "`name`, `city`, `longitude`, `latitude`, `state_code`, `country_sid`";
		}
		...
		if (!$findZipCodes) {
			...
			$isState = SJB_DB::query("SELECT `sid` FROM `states` WHERE `state_name` = ?s OR `state_code` = ?s", $locationValue, $locationValue);
			$query = " `{$table}`.{$id}_City = '{$locationValue}' ";
			if ($isCountry) {
				$query .= " OR `{$table}`.{$id}_Country = '{$isCountry}' ";
			}
			if (!empty($isState)) {
				$states = array();
				foreach ($isState as $state) {
					$states[] = $state['sid'];
				}
				$states = implode(', ', $states);
				$query .= " OR `{$table}`.{$id}_State IN ({$states}) ";
			}
			if ($isState || $isCountry) {
				return " ( " . $query . " ) ";
			}
		} elseif (count($locationValues) == 3) {
			$findZipCodes = SJB_DB::query("SELECT {$selectFields} FROM `locations` WHERE FIND_IN_SET(`name`, ?s) AND FIND_IN_SET(`city`, ?s) AND (FIND_IN_SET(`state`, ?s) OR FIND_IN_SET(`state_code`, ?s))", $value, $value, $value, $value);
			if ($findZipCodes && count($findZipCodes) == 1) {
				$city = !empty($findZipCodes[0]['city']) ? $findZipCodes[0]['city'] : '';
				$stateSID = !empty($findZipCodes[0]['state_code']) ? SJB_StatesManager::getStateSIDByStateCode($findZipCodes[0]['state_code']) : '';
			}
		}
		if (!$findZipCodes) {
			if (count($locationValues) == 1) {
				...
				$stateSID = SJB_DB::query("SELECT `sid` FROM `states` WHERE `state_name` = ?s OR `state_code` = ?s", $locationValues[1], $locationValues[1]);
				if (!empty($stateSID)) {
					$sids = array();
					foreach ($stateSID as $state) {
						$sids[] = $state['sid'];
					}
					$stateSID = implode(', ', $sids);
				}
				if ($findZipCodes && empty($this->value['radius'])) {
					return " (`{$table}`.{$id}_City = '{$city}' AND `{$table}`.{$id}_State IN ({$stateSID})) ";
				}
			}
		...
		if ($findZipCodes) {
			...
			if ($city) {
				if (!empty($stateSID)) {
					$city = "(`{$table}`.{$id}_City = '{$city}' AND `{$table}`.{$id}_State IN ({$stateSID}))";
				} else {
