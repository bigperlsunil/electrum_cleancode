...
class SJB_ListingDBManager extends SJB_ObjectDBManager
{
	...
	public static function saveListing($listing, $listingSidsForCopy = array())
	{
		...
		if (!is_null($listing_type_sid)) {
			...
			if ($expirationDate && !$listing->getSID()) {
	...
	public static function activateListingBySID($listing_sid, $date = false)
	{
		...
		if (SJB_DB::query("UPDATE `listings` SET `active` = 1, `activation_date` = {$activation_date} WHERE `sid` = ?n", $listing_sid)) {
			...
			if ($featuredPeriod) {
				$sql[] = " `featured_expiration` = {$activation_date} + INTERVAL {$featuredPeriod} DAY ";
			}
			if ($priorityPeriod) {
				$sql[] = " `priority_expiration` = {$activation_date} + INTERVAL {$priorityPeriod} DAY ";
			}
			$sql = implode(', ', $sql);
			if ($sql) {
	...
	public static function deactivateListingBySID($listingSID, $deleteRecordFromActivePeriod = false)
	{
		if (SJB_DB::query('UPDATE `listings` SET `active` = 0 WHERE `sid` = ?n', $listingSID)) {
			if ($deleteRecordFromActivePeriod) {
				...
				SJB_DB::query('UPDATE `listings` SET `featured` = 0, `priority` = 0 WHERE `sid`=?n', $listingSID);
			} else {
	...
	{
		// Не самый эффективный способ выключения. но зато простой
		if (SJB_System::getSettingByName('use_autocomplete_for_keywords')) {
	...
	{
		if (SJB_System::getSettingByName('use_autocomplete_for_keywords')) {
			SJB_DB::query('UPDATE `listings_keywords` SET `active` = 1 WHERE `listing_sid` = ?s', $listingSID);
		}
	}
	...
	{
		if (SJB_System::getSettingByName('use_autocomplete_for_keywords')) {
			SJB_DB::query('UPDATE `listings_keywords` SET `active` = 0 WHERE `listing_sid` = ?s', $listingSID);
		}
	}
