...
class SJB_ListingManager extends SJB_ObjectManager
{
	...
	public static function createTemplateStructureForListing($listing, $extraInfo = array())
	{
				...
				foreach ($properties as $subProperty) {
					if (!empty($listing_info['user_defined'][$property->getID()][$subProperty->getID()]) && is_array($listing_info['user_defined'][$property->getID()][$subProperty->getID()])) {
						foreach ($listing_info['user_defined'][$property->getID()][$subProperty->getID()] as $subValue) {
							if ((!empty($subValue) && $subProperty->getType() != 'monetary') || ($subProperty->getType() == 'monetary' && !empty($subValue['value']))) {
        ...
        (
			...
			'priceForUpgradeToFeatured' => floatval($priceForUpgradeToFeatured),
			'priceForUpgradeToPriority' => floatval($priceForUpgradeToPriority),
        );
	...
	public static function getLastListings($number_of_listings, $listing_type)
	{
		...
		$listings_info = SJB_DB::query("SELECT l.*, lt.id FROM listings as l
				...
				WHERE lt.id = ?s AND l.active = 1 $approve_status $sqlAccess ORDER BY  l.activation_date DESC LIMIT 0, ?n", $listing_type, $number_of_listings);
	...
	public static function incrementViewsCounterForListing($listingId, $listing)
	{
		...
		if (empty($listingViews) || !self::isViewCanBeCounted($listingId, $ipAddress)) {
			return false;
		}
	...
	public static function copyFilesAndPicturesFromListing($srcListingSid, $dstListingSid, $tmpListingSid, $copyFileFields = array())
	{
		$listing = SJB_ListingManager::getObjectBySID($srcListingSid);
		if ($listing) {
			foreach ($listing->getProperties() as $listingProperty) {
				if ((isset($copyFileFields[$listingProperty->getID()]) && $copyFileFields[$listingProperty->getID()] != 'do_not_copy') || empty($copyFileFields)) {
					if ($listingProperty->getType() == 'complex') {
						$copyFileFields = !empty($copyFileFields[$listingProperty->getID()]) ? $copyFileFields[$listingProperty->getID()] : array();
						self::copyComplexFiles($dstListingSid, $listingProperty, $copyFileFields);
					}
	...
	private static function copyComplexFiles($dstListingSid, $listingProperty, $copyFileFields = array())
	{
		...
		foreach ($complexFields as $complexField) {
			if ($complexField->getType() == 'complexfile') {
				foreach ($complexField->getValue() as $key => $value) {
					if (isset($copyFileFields[$complexField->getID()][$key]) || empty($copyFileFields)) {
