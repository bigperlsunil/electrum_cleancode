...
class SJB_Notifications
{
	...
	public static function sendApplyNow($info, $file = '', $data_resume = array(), $current_user_sid = false, $notRegisterUserData = false, $score = false)
	{
		...
		if (!empty($info['listing']['subuser']['sid']) && empty($application_email)) {
	...
	public static function sendUserApplicationApproveOrRejectLetter($application_id, $mode = 'approved')
	{
		...
		//application from guest
		if ($application_info['jobseeker_id'] == 0) {
			$userGroupSID = SJB_UserGroupManager::getUserGroupSIDByID('JobSeeker');
			$user_info = array(
				'username' => $application_info['username'],
				'email' => $application_info['email'],
				'registration_date' => $application_info['date']
			);
			$user = new SJB_User($user_info, $userGroupSID);
		} else {
