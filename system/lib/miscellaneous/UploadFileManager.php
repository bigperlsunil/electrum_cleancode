...
class SJB_UploadFileManager
{
	...
	function isValidUploadedFile($fileId, $subField = false, $acceptableFormats = array(), $complexStep = false)
	{
		...
		if (!empty($this->max_file_size) && floatval($this->max_file_size) > 0) {
			if (!$subField) {
				...
				foreach ($_FILES[$fileId]['name'][$subField] as $key => $name) {
					if ($key == $complexStep) {
				...
				foreach ($_FILES[$fileId]['size'][$subField] as $key => $size) {
					if ($key == $complexStep) {
						if ($size > ($this->max_file_size * 1024 * 1024) || $_FILES[$fileId]['error'][$subField][$key] == 1) {
							$this->error = 'MAX_FILE_SIZE_EXCEEDED';
							return false;
						}
					}
	...
	function uploadFile($file_id, $complexParentFieldId = false, $complexStep = false)
	{
		...
		elseif (!empty($_FILES[$file_id]['name'])) {
			...
			if ($_FILES[$complexParentFieldId]['error'][$file_id][$complexStep]) {
				$this->error = $this->getErrorId($_FILES[$complexParentFieldId]['error'][$file_id]);
				return false;
			}

			list($file_name, $saved_file_name) = self::getArrayOfFileNames($_FILES[$complexParentFieldId]['name'][$file_id][$complexStep]);

			if (!empty($_FILES[$complexParentFieldId]['import'][$file_id][$complexStep])) {
				$saved_file = @copy($_FILES[$complexParentFieldId]['tmp_name'][$file_id][$complexStep], $file_name);
			} else {
				$saved_file = move_uploaded_file($_FILES[$complexParentFieldId]['tmp_name'][$file_id][$complexStep], $file_name);
			}

			if ($saved_file) {
				SJB_UploadFileManager::deleteUploadedFileByID($this->uploaded_file_id);
				$this->fileId = SJB_DB::query("INSERT INTO uploaded_files(id, file_name, file_group, saved_file_name, mime_type, creation_time)"
					. " VALUES(?s, ?s, ?s, ?s, ?s, ?s)", $this->uploaded_file_id, $_FILES[$complexParentFieldId]['name'][$file_id][$complexStep], $this->file_group, $saved_file_name, $_FILES[$complexParentFieldId]['type'][$file_id][$complexStep], time());
				return $saved_file_name;
			}
		}
	}
