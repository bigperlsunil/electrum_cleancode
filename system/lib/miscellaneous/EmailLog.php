...
class SJB_EmailLog extends SJB_Object
{
	...
	public static function writeToLog($email, $result = false, $error_msg = false)
	{
		...
		$recipientEmails = array($email->recipient_email);
		foreach ($email->getCarbonCopy() as $copyEmail) {
			$recipientEmails[] = $copyEmail;
		}
		$recipientEmails = implode(', ', $recipientEmails);
		SJB_DB::query("INSERT INTO `email_log` (`date`, `subject`, `email`, `message`, `username`, `admin`, `status`, `error_msg`) VALUES (NOW(), ?s, ?s, ?s, ?s, ?s, ?s, ?s)", $email->subject, $recipientEmails, $email->text, $username, $admin, $status, $error_msg);
	}
}
