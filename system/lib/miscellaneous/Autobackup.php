...
class SJB_Autobackup
{
	...
	{
		$this->lastAutobackup = date('Y-m-d H:i');
...
<IfModule !mod_authz_core.c>
	<FilesMatch ".*">
		...
		Deny from all
				...
				SJB_Settings::updateSetting('last_autobackup', $this->lastAutobackup);
			}
		}
	}
	...
	private function isAutobackup()
	{
		...
		switch ($backupFrequency) {
			case 'daily':
				if (!empty($lastBackup)) {
					$expiredDate = date('Y-m-d H:i', strtotime($lastBackup . " + 1 day"));
					if ($expiredDate <= $this->lastAutobackup) {
						return true;
					}
					return false;
				}
			...
			case 'weekly':
				if (!empty($lastBackup)) {
					$expiredDate = date('Y-m-d H:i', strtotime($lastBackup . " + 1 week"));
					if ($expiredDate <= $this->lastAutobackup) {
						return true;
					}
					return false;
			...
			case 'monthly':
				if (!empty($lastBackup)) {
					$expiredDate = date('Y-m-d H:i', strtotime($lastBackup . " + 1 month"));
					if ($expiredDate <= $this->lastAutobackup) {
						return true;
					}
					return false;
