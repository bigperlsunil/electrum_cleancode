...
class SJB_AdminNotifications
{
	...
	public static function isSubAdminNotifiedOnListingAdded($listingTypeId)
	{
		return SJB_SubAdminManager::getIfSubAdminsNotifiedOn('get_notifications_on_' . strtolower($listingTypeId) . 's_added');
	}
	...
	public static function sendAdminListingAddedLetter(SJB_Listing $listing)
	{
		...
		if ($emailTplSID) {
			...
			if ($email) {
				// notify subadmins
				$subAdminsToNotify = self::isSubAdminNotifiedOnListingAdded($listing['type']['id']);
	...
	public static function isSubAdminNotifiedOnListingExpiration($listingTypeId)
	{
		return SJB_SubAdminManager::getIfSubAdminsNotifiedOn('get_notifications_on_' . strtolower($listingTypeId) . 's_expiration');
	}
	...
	public static function sendAdminListingExpiredLetter($listing)
	{
		...
		if ($emailTplSID) {
			...
			if ($email) {
				// notify subadmins
				$subAdminsToNotify = self::isSubAdminNotifiedOnListingExpiration($listing['type']['id']);
				if (is_array($subAdminsToNotify)) {
					foreach ($subAdminsToNotify as $subAdminEmail) {
						$email->addCC($subAdminEmail);
					}
