...
class SJB_StatisticsExportController
{
		...
		foreach ($statistics as $key => $statistic) {
			foreach ($exportProperties as $fieldName => $property) {
				if ($fieldName == 'percent') {
					if ($statistic[$fieldName] == 100) {
						$export_data[$key][$fieldName] = $statistic[$fieldName] . '%';
					} else {
						$export_data[$key][$fieldName] = number_format($statistic[$fieldName], 2, ',', '') . '%';
					}
				}
	...
	{
		SJB_HelperFunctions::makeCSVExportFile($exportData, $exportFileName);
	}
