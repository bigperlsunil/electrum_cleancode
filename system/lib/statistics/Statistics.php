...
class SJB_Statistics
{
	...
	public static function isBot()
	{
		if (is_null(self::$isBot)) {
			...
			$metadataFile = SJB_System::getSystemSettings('EXT_LIBRARY_DIR') . 'vendor/vipx/bot-detect/Vipx/BotDetect/Resources/metadata/extended.yml';
			$detector = new BotDetector($loader, $metadataFile, array(
				'cache_dir' => SJB_System::getSystemSettings('CACHE_DIR') . '/vipx',
				'metadata_dumper_class' => 'Vipx\\BotDetect\\Metadata\\Dumper\\PhpMetadataDumper',
			));
			self::$isBot = $detector->detect(SJB_Request::getUserAgent(), $_SERVER['REMOTE_ADDR']) !== null;
		}
		return self::$isBot;
	}
	...
	public static function addStatistics($event, $type = '', $objectSID = 0, $unique = false, $featured = 0, $priority = 0, $userSID = false, $price = 0, $plugin = '', $reactivate = 0)
	{
		...
		if (self::isBot() && in_array($event, $browsingEvents)) {
			return false;
		} else {
	...
	public static function getGeneralStatistics($period = false, $groupBy = 'day', $filter = array())
	{
		...
		if ($applyEvent) {
			$applicationWhere = $where . " AND `event` = 'apply'";
		}
		...
		$applicationInfo = SJB_DB::query("SELECT *, sum(`count`) as count, sum(`approve`) as approve, sum(`reject`) as reject, {$query}  FROM `statistics` {$applicationWhere} GROUP BY `event`, groupValue ORDER BY `date`");
		$applicationInfoTotal = SJB_DB::query("SELECT *, sum(`count`) as count, sum(`approve`) as approve, sum(`reject`) as reject, 'total'  as groupValue FROM `statistics` {$applicationWhere} GROUP BY `event`");
		$statisticsInfo = SJB_DB::query("SELECT *, sum(`count`) as count, sum(`featured`) as featured, sum(`priority`) as priority, {$query}  FROM `statistics` {$where} GROUP BY `event`, `type`, groupValue ORDER BY `date`");
		$statisticsInfoTotal = SJB_DB::query("SELECT *, sum(`count`) as count, sum(`featured`) as featured, sum(`priority`) as priority, 'total'  as groupValue FROM `statistics` {$where} GROUP BY `event`, `type`");

		$statisticsInfo = array_merge($statisticsInfo, $statisticsInfoTotal, $applicationInfo, $applicationInfoTotal);
		...
		foreach ($nameEvents as $nameEvent => $title) {
			switch ($nameEvent) {
				...
				case 'viewListing':
					foreach ($listingTypes as $listingTypeInfo) {
						...
						if ($nameEvent == 'addListing') {
							...
							if (!empty($dateStatistics[$nameEvent.$listingTypeID]['featured']))
								$graph[$nameEvent.'Featured'.$listingTypeID]['statistic'] = $dateStatistics[$nameEvent.$listingTypeID]['featured'];
							if (!empty($dateStatistics[$nameEvent.$listingTypeID]['priority']))
								$graph[$nameEvent.'Priority'.$listingTypeID]['statistic'] = $dateStatistics[$nameEvent.$listingTypeID]['priority'];
						}
					}
					break;
				case 'addUser':
				...
				case 'apply':
						...
						if (!empty($dateStatistics[$nameEvent]['approve']))
							$graph[$nameEvent.'Approved']['statistic'] = $dateStatistics[$nameEvent]['approve'];
						if (!empty($dateStatistics[$nameEvent]['reject']))
							$graph[$nameEvent.'Rejected']['statistic'] = $dateStatistics[$nameEvent]['reject'];
		...
		if (!empty($fieldInfo['type']) && $fieldInfo['type'] == 'multilist') {
			...
			if ($fieldValues) {
				...
				$total = SJB_DB::query("SELECT sum(s.`count`) as count FROM `listing_field_list` lfl, `listings` l INNER JOIN `statistics` s  ON l.`sid` = s.`object_sid`
										WHERE lfl.`sid` in ({$fieldSIDs}) AND FIND_IN_SET( lfl.`sid` , l.`{$filter}` ) AND s.`event` = 'addListing' AND s.`type` = '{$listingType}' {$where}", $listingType);
	...
	public static function getApplicationsAndViewsStatistics($period, $filter, $sortingField, $sorting_order) 
	{
		...
		elseif ($filter == 'sid') {
			...
			$join = " INNER JOIN `users` u ON l.`user_sid` = u.`sid` ";
			$groupBy = " l.`{$filter}` ";
			$query = ", l.`sid`, l.`Title`, l.`sid` as {$mainGroupBy} , u.`CompanyName`, u.`username`, l.`user_sid` ";
		} else {
			$mainGroupBy = $filter;
			$where .= " AND l.`{$filter}` != '' AND l.`{$filter}` IS NOT NULL ";
			$groupBy = " l.`{$filter}` ";
	...
	public static function getEmployerQuickStatistics($userSID)
	{
		...
		$subQueryResult = SJB_DB::query("SELECT `l`.`sid` FROM `listings` `l` WHERE `l`.`user_sid` = ?n
					 					 UNION ALL
					 					 SELECT `st`.`object_sid` FROM `statistics` `st` WHERE `st`.`user_sid` = ?n AND `st`.`event` = 'deleteListing'", $userSID, $userSID);
		$sids = array();
		foreach ($subQueryResult as $resultSid) {
			$sids[] = $resultSid['sid'];
		}
		if (empty($sids)) {
			$sids[] = '';
		...
		$quickStats = SJB_DB::query("
			SELECT
				IFNULL((SELECT SUM(`s`.`count`)
					FROM `statistics` `s`
					WHERE `s`.`user_sid` = ?n AND `s`.`event` = 'addListing' AND `s`.`type`= 6 {$where} GROUP BY `s`.`user_sid`), 0) AS `countPostedListings`,
				(SELECT count(*)
					FROM `statistics` `s`
					INNER JOIN (
						SELECT `l`.`sid` as sid
							...
							WHERE `l`.`user_sid` = ?n
								UNION ALL
									SELECT `st`.`object_sid`
									FROM `statistics` `st`
									WHERE `st`.`user_sid` = ?n AND `st`.`event` = 'deleteListing'
						) objects on objects.sid = s.object_sid
					WHERE `s`.`event` = 'viewListing' AND `s`.`type`= 6 {$where})
					AS `countViewedListings`,
				IFNULL((SELECT SUM(`s`.`count`)
					FROM `statistics` `s`
					WHERE `s`.`event` = 'apply' {$where} AND `s`.`type` IN (?l)
					GROUP BY `s`.`user_sid`), 0) AS `countApplications`
		", $userSID, $userSID, $userSID, $sids);
		...
		foreach ($periods as $key => $value) {
			$statForPeriod = SJB_DB::query("
				...
				FROM (
					...
					FROM `statistics` `s`
					WHERE `s`.`event` = 'apply' {$value} AND `s`.`type` IN (?l)
					GROUP BY `s`.`user_sid`
					UNION ALL
					...
					GROUP BY `s`.`user_sid`
				) AS `generalStats`", $userSID, $userSID, $userSID, $sids, $userSID);
