		...
		if (!empty($query) && $field && $fieldType && $tablePrefix && $viewType && $listingTypeID) {
            ...
            elseif ($fieldType == 'location') {
				if (preg_match('/[a-z\d]+\d+/i', $query)) {
					$result = SJB_DB::query('SELECT DISTINCT `name` as `value`, `city`, `state_code`, COUNT(*) `count` FROM `locations` WHERE `name` <> \'\' AND `name` LIKE ?s GROUP BY `value`, `city`, `country_sid` LIMIT 0 , 10', $queryCriterion);
            	} else {
