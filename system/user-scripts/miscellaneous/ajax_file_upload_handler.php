	...
	public function execute()
	{
		...
		switch ($ajaxAction) {
			...
			case 'upload_profile_logo':
				...
				if ($validation === true) {
					...
					if ($fieldInfo['type'] == 'video' && isset($this->propertyValue['file_id'])) {
						...
						$uploadFileSID = SJB_DB::queryValue("SELECT `sid` FROM `uploaded_files` WHERE `id` = ?s", $uploadedID);
						if ($uploadFileSID) {
							SJB_DB::query("DELETE FROM `uploaded_files` WHERE `id` = ?s", $this->fileUniqueId);
						}
			...
			case 'upload_file':
				...
				$listingId     = SJB_Request::getVar('listing_id', 0, 'GET');
				$listingTypeId = SJB_Request::getVar('listing_type_id');
				if (!$listingTypeId) {
					$listingTypeId = SJB_Request::getVar('listing_type_id', 0, 'GET');
				}
				...
				if (!$validation) {
					...
					if (isset($this->propertyValue['file_id'])) {
						...
						foreach ($listingProperties as $property) {
							...
							} elseif (in_array($property->getType(), array('video', 'file'))) {
			...
			case 'upload_classifieds_video_complex':
				...
				$upload_manager->setUploadedFileID($this->fileUniqueId);
				$upload_manager->setFileGroup('files');
				$upload_manager->uploadFile($fieldInfo['id'], $parentField, $complexStep);
				$this->property->setValue($this->fileUniqueId);
