...
class SJB_Classifieds_JobImport extends SJB_Function
{
	...
	public function execute()
	{
		...
		if ($action == 'example' && $type) {
			...
			foreach ($listing_fields as $listing_field) {
				if ($listing_field['type'] == 'location') {
					foreach ($listing_field['fields'] as $field) {
						$locationId = $listing_field['id'] . '.' . $field['id'];
						$export_properties[$locationId] = $locationId;
						$export_data[0][$locationId] = '';
					}
				} else {
									...
									elseif ($property->getType() == 'complex' && is_array($property->value)) {
										...
										foreach ($childFields as $childField) {
											if (($childField['type'] == 'complexfile') && !empty($complexChildValues[$childField['id']])) {
												...
												if (isset($listing_info[$property->id][$field_info['id']])) {
													foreach ($listing_info[$property->id][$field_info['id']] as $complexFile) {
														if (file_exists($complexFile)) {
	...
	private function FillGallery($listing, $listing_info)
	{
		...
		if (!empty($listing_info['pictures'])) {
			$pictures = !is_array($listing_info['pictures']) ? explode(';', $listing_info['pictures']) : $listing_info['pictures'];
