	...
	public function execute()
	{
		...
		if ($productsInfo = $this->canCurrentUserAddListing($error, $listing_type_id)) {
			...
			if ($contractID) {
				...
				if (count($pictures_info) <= $numberOfPictures) {
					$gallery->setListingSID($tmp_listing_sid);
					//reuploading pictures
					if (!$gallery->getPicturesInfo()) {
				...
				elseif (!is_null($listing_info)) {
					...
					$copyFileFields = array();
					$formIsSubmitted = $action == 'save_info' || $action == 'add';
					if (!$formIsSubmitted) {
						$listing = new SJB_Listing($listing_info, $listing_info['listing_type_sid']);
					} else {
						$listing = new SJB_Listing($_REQUEST, $listing_info['listing_type_sid']);
					}
					...
					if (empty($extraInfo['video'])) {
						foreach ($listing->getProperties() as $property) {
							if (in_array($property->getType(), array('video', 'youtube'))) {
								$listing->deleteProperty($property->getID());
								$copyFileFields[$property->getID()] = 'do_not_copy';
							}
						}
					}
					...
					elseif ($formIsSubmitted && $listing_edit_form->isDataValid($field_errors)) {
						$listing->addProperty(
							array('id' => 'complete',
								'type' => 'integer',
								'value' => 1,
						...
						foreach ($listing->getProperties() as $listingProperty) {
							if ($listingProperty->getType() == 'complex') {
									...
									foreach ($complexFields as $complexField) {
										if ($complexField->getType() == 'complexfile') {
											foreach ($complexField->value as $complexEnum => $value) {
												if (!empty($listing_info[$listingProperty->getID()][$complexField->getID()][$complexEnum]) && $listing_info[$listingProperty->getID()][$complexField->getID()][$complexEnum] == $value) {
													$copyFileFields[$listingProperty->getID()][$complexField->getID()][$complexEnum] = $value;
												}
											}
										}
									}
							...
							elseif (in_array($listingProperty->getType(), array('file', 'video'))) {
								...
								$value = !empty($value['file_id']) ? $value['file_id'] : false;
								if (!empty($listing_info[$listingProperty->getID()]) && $listing_info[$listingProperty->getID()] == $value) {
									$copyFileFields[$listingProperty->getID()] = $value;
								} else {
									$copyFileFields[$listingProperty->getID()] = 'do_not_copy';
								}
							}
						}
					...
					$contractParentListing = new SJB_Contract(array('contract_id' => $contract_id));
					$tp->assign('form_token', SJB_Request::getVar('form_token'));
					$tp->assign('contract_id', $contract_id);
					$tp->assign('contract', $contractParentListing->extra_info);
					...
					$tp->assign('extraInfo', $contract->extra_info);
					$tp->assign('listing_type_id', $listing_type_id);
