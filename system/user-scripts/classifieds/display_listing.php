	...
	public function execute()
	{
		...
		elseif ((SJB_ListingTypeManager::getListingTypeIDBySID($listing->listing_type_sid) == 'Resume' && ($template == 'display_job.tpl')) ||
			...
			if (!empty($zipCode) && empty($latitude) && empty($longitude)) {
				$result = array();
				$where = array();
				foreach ($listing_structure['Location'] as $fieldName => $fieldValue) {
					if (!empty($listing_structure['Location'][$fieldName])) {
						switch ($fieldName) {
							case 'City':
								$where[] = "`city` = '{$listing_structure['Location']['City']}'";
								break;
							case 'State_Name':
								$where[] = "`state` = '{$listing_structure['Location']['State_Name']}'";
								break;
							case 'Country':
								$countrySID = SJB_CountriesManager::getCountrySIDByCountryName($listing_structure['Location']['Country']);
								$where[] = "`country_sid` = '{$countrySID}'";
								break;
						}
					}
				}
				...
				if ($where) {
					...
					$result = SJB_DB::query("SELECT * FROM `locations` WHERE `name` = ?s AND {$where} LIMIT 1", $zipCode);
				}
				if (!$result) {
			...
			foreach ($errors as $k => $v) {
				switch ($k) {
					...
					case 'LISTING_IS_NOT_APPROVED':
						// todo: remove this variables from templates
