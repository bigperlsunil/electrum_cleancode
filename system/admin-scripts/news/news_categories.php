	...
	public function execute()
	{
		...
		switch ($action) {
			...
			case 'save_display_setting':
				...
				if (!empty($settings['number_news_on_main_page'])) {
					$i18n = SJB_I18N::getInstance();
					if (!$i18n->isValidInteger($settings['number_news_on_main_page'])) {
						$errors['Number of News to Display on Homepage'] = 'NOT_INT_VALUE';
						break;
					} elseif ($settings['number_news_on_main_page'] < 0) {
						$errors['Number of News to Display on Homepage'] = 'NOT_PLUS_VALUE';
						break;
					}
				}
			...
			case 'edit':
				...
				if ($formSubmitted) {
					...
					if (!$this->isEmpty($newCategoryName)) {
						$isExists = SJB_NewsManager::checkExistsCategoryName($newCategoryName, $categoryId);
						if (!$isExists) {
				...
				if ($archiveCategory['id'] == $categoryId) {
					if (isset($paginationInfo['actionsForSelect']['archive'])) {
						unset($paginationInfo['actionsForSelect']['archive']);
					}
				}
