...
class SJB_Admin_StaticContent_EditStaticContent extends SJB_Function
{
	...
	public function execute()
	{
		...
		if (in_array($action, array('add', 'change', 'delete'))) {
			if (SJB_System::getSystemSettings('isDemo') || SJB_System::getIfTrialModeIsOn()) {
				if (SJB_System::getSystemSettings('isDemo')) {
					$errors = SJB_StaticContentAuxil::warning('Error', 'You don\'t have permissions for it. This is a Demo version of the software.');
				} else {
					$errors = SJB_StaticContentAuxil::warning('Error', 'You don\'t have permissions for it. This is a Trial version of the software.');
				}
				if ($action == 'change') {
					$action = 'edit';
				} else {
