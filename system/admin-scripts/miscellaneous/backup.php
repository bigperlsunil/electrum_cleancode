		...
		switch ($action) {
			...
			case 'backup':
				if (SJB_System::getSystemSettings('isDemo')) {
					...
					$str = $i18N->gettext('Backend', 'You don\'t have permissions for it. This is a Demo version of the software.');
				...
				if (SJB_System::getIfTrialModeIsOn() && $_SERVER['REMOTE_ADDR'] != "91.205.51.231") {
					$i18N = SJB_I18N::getInstance();
					$str = $i18N->gettext('Backend', 'You don\'t have permissions for it. This is a Trial version of the software.');
					SJB_Session::setValue('error', $str);
					break;
				}
			...
			case 'restore':
				...
				if (SJB_System::getSystemSettings('isDemo')) {
					SJB_Session::setValue('error', 'You don\'t have permissions for it. This is a Demo version of the software.');
					exit();
				}

				if (SJB_System::getIfTrialModeIsOn()) {
					SJB_Session::setValue('error', 'You don\'t have permissions for it. This is a Trial version of the software.');
					exit();
				}
				...
				if (is_dir($path)) {
					...
					foreach ($di as $file) {
						...
						if (!$file->isDir() && !$file->isLink() && $fileName != '.htaccess') {
							...
							$pattern = '/(\w+)_(\d+)_(\d+)_(\d+)__(\d+)_(\d+).tar.gz/iu';
