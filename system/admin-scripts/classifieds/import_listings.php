		...
		if (empty($file_info) || $file_info['error'] || $errors) {
			...
			foreach ($imported_data as $key => $importedColumn) {
				...
				if (!$importedColumn)
					continue;
				$count++;
				$listingInfo = $importedDataProcessor->getData($non_existed_values_flag, $importedColumn);
				$doc = new DOMDocument();
				...
				if ($listing->getUserSID()) {
					...
					SJB_Statistics::addStatistics('addListing', $listing->getListingTypeSID(), $listingSid, false, $productInfo['featured'], $productInfo['priority']);
					if ($listing->isActive()) {
