...
class SJB_Admin_Classifieds_EditListing extends SJB_Function
{
	...
	public function execute()
	{
		...
		if (!is_null($listing_info)) {
			...
			$screening_questionnaires = SJB_ScreeningQuestionnaires::getListSIDsAndCaptions($listing->user_sid);
			if (SJB_Acl::getInstance()->isAllowed('use_screening_questionnaires') && $screening_questionnaires) {
				$listing->details->getProperty('screening_questionnaire')->type->list_values = $screening_questionnaires;
			} else {
