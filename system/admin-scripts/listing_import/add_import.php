		...
		switch ($add_level) {
			...
			case '3':
				...
				if ($addNewUser == 1 && empty($parserInfo['mapped_user'])) {
					...
					SJB_HelperFunctions::redirect(SJB_System::getSystemSettings('SITE_URL') . '/edit-import/?id=' . $id. '&save_error='.base64_encode($error));
				}
			...
			case '2':
				...
				$selectedProduct    = SJB_Request::getVar('postUnderProduct');
				$id                 = SJB_Request::getVar('id', 0, 'GET');
				$script             = SJB_Request::getVar('custom_script', '');
				$script_users       = SJB_Request::getVar('custom_script_users', '');
				...
				$tp->assign('custom_script', $script);
				$tp->assign('custom_script_users', $script_users);
