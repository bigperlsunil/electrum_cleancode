		...
		if (!empty ($_REQUEST ['xml']) || $id > 0) {
			if ($id > 0) { // load exist parser
				...
				if ($formIsSubmitted) {
					...
					if (empty($productInfo['product_type']) || !in_array($productInfo['product_type'], array('post_listings', 'mixed_product'))) {
						$errors['Post under product'] = 'NOT_VALID_VALUE';
					}
					...
					if ($addNewUser == 1 && empty($parserInfo['mapped_user'])) {
						$errors[] = 'Required user profile fields are not mapped';
					}
				...
				$custom_script       = $parserInfo ['custom_script'];
				$custom_script_users = $parserInfo ['custom_script_users'];
				$add_new_user        = $parserInfo ['add_new_user'];
				$importType          = $parserInfo ['import_type'];
				$xml                 = $parserInfo ['xml'];
				$xml_logo            = $parserInfo ['xml_logo'];
				...
				$selected_logo_options = $parserInfo['logo_options'];

				if (!$formIsSubmitted) {
					$xml = SJB_XmlImport::cleanXmlFromImport($xml);
					$selected_logo_options = unserialize($selected_logo_options);
					$selectedLogoOption  = !empty($selected_logo_options['option']) ? $selected_logo_options['option'] : '';
					$selectedLogoField   = !empty($selected_logo_options['field']) ? $selected_logo_options['field'] : '';
					$defaultValue = ($defaultValue != '') ? unserialize($defaultValue) : array();
				} else {
					$selectedLogoOption = $selected_logo_options;
					$selectedLogoField =  !empty($parserInfo['import_logo_field']) ? $parserInfo['import_logo_field'] : '';
				}
				
				if ($selectedLogoOption == 'upload_logo') {
