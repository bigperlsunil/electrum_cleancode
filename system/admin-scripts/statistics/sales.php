	...
	public function execute()
	{
		...
		switch ($action) {
				...
				if (!$errors && $statistics) {
					...
					switch ($type) {
						case 'csv':
							...
							$fileName = 'sales.csv';
							SJB_StatisticsExportController::makeCSVExportFile($exportData, $fileName);
							SJB_StatisticsExportController::archiveAndSendExportFile('sales', 'csv');
							break;
						case 'xls':
							$exportData = SJB_StatisticsExportController::getSalesExportData($statistics, $exportProperties);
							$fileName = 'sales.xls';
							SJB_StatisticsExportController::makeXLSExportFile($exportData, $fileName, 'Sales');
			...
			case 'search':
				...
				$dateFrom = !empty($period['from']) ? strtotime($i18n->getInput('date', $period['from'])) : false;
				$dateTo = !empty($period['to']) ? strtotime($i18n->getInput('date', $period['to'])) : false;

				if($search) {
					if (!empty($period['from']) && !empty($period['to']) && ($dateFrom > $dateTo))
